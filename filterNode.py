...
def mask_nan_info(mask, n):
    """ Sets values to False if there are more than n consecutive True values and
    also returns information about location of gaps
    """
    gaps = []
    i=0
    while i < (len(mask)-n-1):
...
def update_gap_information(data):
    """
    update gap_info file on aws with gaps generated by this run of filter
    """
    global MAX_GAP

    mask = np.isnan(data)
    # find location of gaps
    # this is slightly redundant but the easiest way to incorportae into the 
    # rest of the code
    mask, gaps = mask_nan_info(mask, MAX_GAP)

    # NOTE: what information the model requires depends on the way the ml handler 
    # uses the data, for now I will leave what I had originally but there is 
    # redundancy (for example the input data doesn't need to be saved if it is 
    # loaded later and vice versa). This should be updated along with the ml handler. 
    pre_length  = 24 # number of historical data points to include
    
    # generate new gap information dictionary
    info = {"gaps": []}
    for gap in gaps:
        gap_length = gap[1]-gap[0]

        if(gap[0] > pre_length):
            pre_data = data[gap[0]-pre_length:gap[0]] # data for pre_length data points before the gap
            pre_index = input_dates[gap[0]-pre_length:gap[0]] # corrosponding dates
        else: 
            # if there are fewer than pre_length datapoints include all of them
            pre_data = data[0:gap[0]] 
            pre_index = input_dates[0:gap[0]]

        gap_index = input_dates[gap[0]:gap[1]]

        info["gaps"].append({'length': gap_length, "input": pre_data.tolist(), 
                        "input_dates": pre_index.tolist(),  "pred_dates": gap_index.tolist()})

    # convert to json, need to read dump and redump to append

    
    # past_info = ea.getFileAWSJSON('', '', 'gap_info.json')

    # for testing
    # In deployment file needs to be on s3 isntead as above
    gap_info_file = open("gap_info.json", "r")
    past_info = json.load(gap_info_file)

    try:
        past_info["gaps"].append(info["gaps"][0])
    except KeyError:
    ...
    # upload to s3
    # ea.uploadDirectAWSJSON('', '', "gap_info.json", past_info):

    # for testing, in deployment need to put back on s3 as above
...
def run_filter(input_data, upper_threshold, lower_threshold, changing_rate,  
    ...
    update_gap_information(rate_filtered_data)
